...
export const SEED_PARAMETERS: readonly LegalParameterInput<unknown>[] = [
  ...
  // -------------------------------------------------------------------------
  // Пеня за просрочку уплаты (§11)
  // -------------------------------------------------------------------------
  {
    // Кратность к базовой ставке НБ РК — стабильная норма НК.
    key: 'penalty.multiplier',
    value: Rate.percent(125),
    validFrom: FROM_2026,
    validTo: null,
    norm: 'ст. 104 НК РК — пеня 1,25-кратная базовая ставка НБ РК за каждый день',
    source: NK_2026,
    todoVerify: false,
  },
  {
    // Базовая ставка меняется решениями НБ РК — значение на дату не подтверждено.
    key: 'penalty.nbrk.base.rate',
    value: Rate.percent('16.5'),
    validFrom: FROM_2026,
    validTo: null,
    norm: 'базовая (официальная) ставка НБ РК — уточнять на дату просрочки',
    source: 'подтвердить у эксперта (решение НБ РК)',
    todoVerify: true,
  },
